...
class AndroidVariables {
    ...
    addVariable(varinfo) {
        const variablesReference = ++this.nextId;
    ...
    _getObjectIdReference(type, objvalue) {
        ...
        if (!this.objIdCache[key]) {
            this.objIdCache[key] = ++this.nextId;
        }
        return this.objIdCache[key];
    }
    ...
    getVariables(variablesReference) {
        ...
        if (!varinfo) {
            ...
            const range = varinfo.range, count = range[1] - range[0];
            ...
            if (count > 110) {
                ...
                const subrangelen = Math.max(Math.pow(10, (Math.log10(count)|0)-1),100), variables = [];
                for (let i = range[0], varref, v; i < range[1]; i+= subrangelen) {
                    varref = ++this.nextId;
                    v = this.variableHandles[varref] = {
                        varref,
                        arrvar: varinfo.arrvar,
                        range: [i, Math.min(i+subrangelen, range[1])],
                    };
                    variables.push({
                        name:`[${v.range[0]}..${v.range[1]-1}]`,
                        type: '',
                        value: '',
            ...
            const variables = [];
            const bits = {J:64,I:32,S:16,B:8}[varinfo.signature];
    ...
    _local_to_variable(v) {
        ...
        const formats = {};
        const typename = v.type.package ? `${v.type.package}.${v.type.typename}` : v.type.typename;
        switch(true) {
    ...
    setVariableValue(args) {
        ...
        const v = this.variableHandles[args.variablesReference];
        if (!v || !v.cached) {
            return failSetVariableRequest(`Variable '${args.name}' not found`);
        }

        const destvar = v.cached.find(v => v.name === args.name);
        ...
        switch(true) {
            ...
            case !!(m = value.match(/^[+-]?[0-9]+([eE][+]?[0-9]+)?$/)):
                ...
                switch(true) {
                    ...
                    default:
                        ...
                        data = {
                            valuetype:'long',
                            value:NumberBaseConverter.decToHex(value, 16),
                        };
                        switch(true){
            ...
            case !!(m = value.match(/^[+-]?[0-9]*\.[0-9]+(?:[eE][+-]?[0-9]+)?([dDfF])?$/)):
                ...
                data = {
                    valuetype: /^[dD]$/.test(m[1]) ? 'double': 'float',
                    value: num,
                }; 
                break;
            case !!(m = value.match(/^'(?:\\u([0-9a-fA-F]{4})|\\([bfrntv0'])|(.))'$/)):
                // character literal
                const cvalue = m[1] ? String.fromCharCode(parseInt(m[1],16)) : 
                    m[2] ? {b:'\b',f:'\f',r:'\r',n:'\n',t:'\t',v:'\v',0:'\0',"'":"'"}[m[2]]
                    : m[3]
                data = {
                    valuetype: 'char',
        ...
        if (!rootvar) {
            ...
            if (data.valuetype !== 'oref' && destvar.type.signature === JTYPES.String.signature) {
                ...
                let is_in_range = (validmap[destvar.type.signature]||'').indexOf(data.valuetype) >= 0;
        ...
        return rootvar.then(data => {
            ...
            switch(destvar.vtype) {
                ...
                case 'arrelem': 
                    const idx = parseInt(args.name, 10);
                    const count = 1;
                    if (idx < 0 || idx >= destvar.data.arrobj.arraylen) {
        ...
        .then(newlocalvar => {
            ...
            return this._local_to_variable(destvar);
        })
        .catch(e => {
